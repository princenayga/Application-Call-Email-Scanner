...
OUTPUT_HTML = os.path.join("output", "implementation_guide.html")
...
<head>
<meta charset="UTF-8">
<title>Science Corps Fellowship Scanner — Implementation Guide</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .cover-org  {{ font-size: 13px; color: #555; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 16px; }}
  .cover-title {{ font-size: 28px; font-weight: bold; color: #1F4E79; margin-bottom: 10px; line-height: 1.3; }}
  .cover-sub  {{ font-size: 15px; color: #444; margin-bottom: 30px; }}
  .cover-meta {{ font-size: 12px; color: #777; }}

  /* ── Table of contents ── */
  .toc {{ background: #f0f5fb; border-radius: 8px; padding: 24px 30px; margin-bottom: 36px; }}
  .toc h2 {{ font-size: 14px; color: #1F4E79; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.6px; }}
  .toc ol  {{ padding-left: 20px; }}
  .toc li  {{ margin-bottom: 5px; font-size: 13px; color: #333; }}
  ...
  .step p, .step li {{ color: #333; margin-bottom: 4px; }}
  .step ul, .step ol {{ padding-left: 20px; margin-top: 6px; }}
  ...
  .callout-tip    {{ background: #e8f5e9; border-left: 4px solid #2e7d32; color: #1b4f20; }}
  .callout-warn   {{ background: #fff8e1; border-left: 4px solid #f57f17; color: #5a3a00; }}
  .callout-info   {{ background: #e3f0fb; border-left: 4px solid #1F4E79; color: #1a3a5c; }}
  .callout-danger {{ background: #fde8e8; border-left: 4px solid #c62828; color: #6a1010; }}
  ...
  td {{ padding: 8px 12px; border-bottom: 1px solid #dde6f0; vertical-align: top; }}
  tr:nth-child(even) td {{ background: #f4f8fc; }}
  td:first-child {{ font-family: monospace; font-size: 12px; color: #1a3a5c; white-space: nowrap; }}
...
<!-- ══════════════════════════════════════════════════════ COVER -->
<div class="cover">
  <div class="cover-org">Science Corps — Philippine Manager's Office</div>
  <div class="cover-title">Fellowship Scanner<br>Implementation Guide</div>
  <div class="cover-sub">Step-by-step workflow for the email blast analytics pipeline</div>
  <div class="cover-meta">Prepared for: Prince Nayga &nbsp;|&nbsp; {GENERATED}</div>
</div>
...
<div class="toc">
  ...
  <ol>
    ...
    <li>Cleaning the Listserv for the Next Cycle</li>
    <li>Full File Reference</li>
...
<!-- ══════════════════════════════════════════════════════ PART 0 -->
<div class="part">1 &nbsp;—&nbsp; What This System Does</div>
...
<div class="callout callout-info">
  <strong>Three scripts, three outputs</strong>
  <code>fellowship_scanner.py</code> → main Excel report &nbsp;|&nbsp;
  <code>positive_replies_report.py</code> → outreach HTML/PDF &nbsp;|&nbsp;
  <code>clean_listserv.py</code> → cleaned listserv Excel
</div>


<!-- ══════════════════════════════════════════════════════ PART 1 -->
<div class="part">2 &nbsp;—&nbsp; One-Time Setup</div>

<p>Do this once on each machine you use. Skip any step you have already completed.</p>

<div class="step">
  <div class="step-title"><span class="step-num">1</span> Get the project files</div>
  <p>Clone or copy the project folder to your Desktop (or any folder you prefer).</p>
  <p>The folder should contain: <code>fellowship_scanner.py</code>, <code>clean_listserv.py</code>,
     <code>positive_replies_report.py</code>, <code>requirements.txt</code></p>
</div>

<div class="step">
  <div class="step-title"><span class="step-num">2</span> Create and activate a Python virtual environment</div>
  <p>Open a terminal in the project folder and run:</p>
  <div class="cmd-block">python -m venv venv
venv\\Scripts\\activate</div>
  <p>Your prompt will show <code>(venv)</code> when the environment is active. Always activate it before running any script.</p>
</div>
...
<div class="step">
  <div class="step-title"><span class="step-num">4</span> Get your Google OAuth credentials</div>
  <ol>
    <li>Go to <strong>console.cloud.google.com</strong> and sign in with <code>pnayga@science-corps.org</code></li>
    <li>Select your project (or create one named <em>Fellowship Scanner</em>)</li>
    <li>Go to <strong>APIs &amp; Services → Library</strong> → enable <strong>Gmail API</strong></li>
    ...
    <li>Click the download icon → save the file as <code>credentials.json</code> in the project folder</li>
  </ol>
  <div class="callout callout-danger" style="margin-top:10px;">
    <strong>Never share or commit credentials.json</strong>
...
<div class="step">
  <div class="step-title"><span class="step-num">2</span> Update the CONFIG block in fellowship_scanner.py</div>
  <p>Open <code>fellowship_scanner.py</code> in any text editor and update these values near the top:</p>
  <table>
...
<h3>Sheet 3 — Replies</h3>
<p>Every email and listserv address, one row each, colour-coded:</p>

<div class="legend">
  <span class="chip" style="background:#C6EFCE;">Positive / Interested</span>
  <span class="chip" style="background:#FFFF99;">Has a Question</span>
  <span class="chip" style="background:#BDD7EE;">Application Submitted</span>
  <span class="chip" style="background:#FFEB9C;">No Longer Works There</span>
  <span class="chip" style="background:#FFC7CE;">Bounce / Delivery Failure</span>
...
<div class="step">
  <div class="step-title"><span class="step-num" style="background:#595959;">3</span> STEP 3 — INCLUDE IN NEXT BLAST &nbsp;<em>(No Reply, Auto-Reply)</em></div>
  <p>No action needed now. These addresses are valid — people either did not see the email or were out of office. They will be included in the cleaned listserv for the next cycle automatically.</p>
</div>
...
<div class="step">
  ...
  <div class="cmd-block">python positive_replies_report.py</div>
  <p>This reads <code>output/fellowship_replies.xlsx</code> and generates <code>output/positive_replies_outreach.html</code>.</p>
</div>

<div class="step">
  <div class="step-title"><span class="step-num">2</span> Convert to PDF</div>
  <ol>
    <li>Open <code>output/positive_replies_outreach.html</code> in Chrome or Edge</li>
...
<div class="step">
  <div class="step-title"><span class="step-num">3</span> Review and personalise before sending</div>
...
<div class="step">
  ...
  <div class="cmd-block">python clean_listserv.py</div>
  <p>This generates <code>output/cleaned_listserv.xlsx</code> with three sheets:</p>
  <table>
    <tr><th>Sheet</th><th>Contents</th><th>What to do with it</th></tr>
    <tr><td>Cleaned Listserv</td><td>Addresses with confirmed dead bounces removed</td><td>Use this as your listserv for the next blast</td></tr>
    <tr><td>Needs Review</td><td>No Longer Works There + Declined rows with summaries</td><td>Read each row and decide: remove, update, or follow up</td></tr>
    <tr><td>Removed (Bounces)</td><td>Confirmed dead addresses</td><td>Keep for your records; do not re-add to any list</td></tr>
  </table>
</div>
...
<h3>One-time setup (first machine only)</h3>
<ul style="padding-left:22px;line-height:2;">
  <li>&#9744; Create Python virtual environment and install requirements.txt</li>
  <li>&#9744; Download credentials.json from Google Cloud Console</li>
  <li>&#9744; Run scanner once to trigger Gmail browser login (token.json saved)</li>
</ul>
...
<div class="callout callout-tip" style="margin-top:20px;">
  <strong>Tip — Re-running the scanner</strong>
  You can run <code>fellowship_scanner.py</code> multiple times safely. New replies are appended;
  existing rows are never duplicated. Run it again after a few days to catch late replies.
</div>

<div class="footer">
  Science Corps Fellowship Scanner &nbsp;|&nbsp; Implementation Guide &nbsp;|&nbsp; {GENERATED} &nbsp;|&nbsp; Prince Nayga &lt;pnayga@science-corps.org&gt;
</div>
